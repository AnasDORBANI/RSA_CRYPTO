...
class database_manager:
    ...
    def __init__(self):
        ...
        self.cur.execute("CREATE TABLE IF NOT EXISTS connexion(utilisateur VARCHAR PRIMARY KEY, mot_de_passe VARCHAR)")

        self.cur.execute("CREATE TABLE IF NOT EXISTS donnees_cryptage(utilisateur VARCHAR,date DATE,fichier,dictionnaire de cles,FOREIGN KEY (utilisateur) REFERENCES connexion(utilisateur))")
    ...
    def partie_objet_tableau(self, nom_tableau, user = '',colonne = 0):
        ...
        if nom_tableau == 'connexion':
            ...
            self.cur.execute(f'SELECT * FROM {nom_tableau} WHERE utilisateur = (?)',(user,))
    ...
    def trouver_objet(self, nom_tableau, objet, colonne):
        ...
        self.cur.execute(f'SELECT * FROM {nom_tableau} WHERE "{colonne}" = (?)',(objet,))
    ...
    def supprimer_objet(self, nom_tableau, colonne, objet):
        ...
        self.cur.execute(f'DELETE FROM {nom_tableau} WHERE {colonne} = (?)',(objet,))
...
class fenetre1(Frame):
    ...
    def mainWidgets(self):
        ...
        self.utilisateur_lb = Label(self,text = 'Utilisateur',bg = '#FFFFFF',font = ('arial', 10, 'bold'))

        self.utilisateur = StringVar()

        self.utilisateur_en = ttk.Combobox(self, values = bd.partie_objet_tableau('connexion')[0], textvariable = self.utilisateur)

        #Partie de mot de passe----------------------------------------------------------------------------------------------------------

        self.mot_de_passe = StringVar()

        self.mot_de_passe_lb = Label(self,text = 'Mot de passe',bg = '#FFFFFF',font = ('arial', 10, 'bold'))

        self.mot_de_passe_en = ttk.Entry(self, show = '*', textvariable=self.mot_de_passe)

        #partie bouton de connection-----------------------------------------------------------------------------------------------------

        self.se_connecter_btn = ttk.Button(self, text = 'Se connecter',command  = self.se_connecter)

        self.inscrire_btn = ttk.Button(self, text = "S'inscrire",command  = self.inscrire)

        self.supprimer_btn = ttk.Button(self, text = "Supprimer compte",command  = self.supprimer)
        ...
        if self.utilisateur_en.get() == '' or self.mot_de_passe_en.get() == '' :
            ...
            for donnee in bd.partie_objet_tableau('connexion')[1]:
                ...
                if self.utilisateur_en.get() == donnee[0] and hashlib.md5(self.mot_de_passe_en.get().encode()).hexdigest() == donnee[1]:
                    ...
                    test = messagebox.askokcancel('Information', 'Voulez vous continuer?')

                    if test == True:

                        self.clear_frame()

                        self.fenetre_cryptage = fenetre3(self.parent, bd, user, self.mot_de_passe.get())
                ...
                messagebox.showerror('Alert', "Nom d'utilisateur ou mot de passe est incorrect")
        ...
        if self.utilisateur_en.get() == '' or self.mot_de_passe_en.get() == '' :
            ...
            for donnee in bd.partie_objet_tableau('connexion')[1]:
                ...
                if self.utilisateur_en.get() == donnee[0] and hashlib.md5(self.mot_de_passe_en.get().encode()).hexdigest() == donnee[1]:
                    ...
                    test = messagebox.askokcancel('Alert', 'La suppression est definitive \nVoulez vous continuer?')

                    if test == True:

                        bd.supprimer_objet('connexion','utilisateur' , self.utilisateur_en.get())
                        
                        bd.supprimer_objet('donnees_cryptage','utilisateur' , self.utilisateur_en.get())
...
class fenetre2(Frame):
    ...
    def mainWidgets(self):
        ...
        self.utilisateur_ins_lb = Label(self,text = 'Utilisateur',bg = '#FFFFFF',font = ('arial', 10, 'bold'))

        self.utilisateur_ins = StringVar()

        self.utilisateur_ins_en = ttk.Entry(self, textvariable = self.utilisateur_ins)

        #Partie de mot de passe----------------------------------------------------------------------------------------------------------

        self.mot_de_passe_ins = StringVar()

        self.mot_de_passe_ins_lb = Label(self,text = 'Mot de passe',bg = '#FFFFFF',font = ('arial', 10, 'bold'))

        self.mot_de_passe_ins_en = ttk.Entry(self, show = '*', textvariable=self.mot_de_passe_ins)

        self.conf_mot_de_passe_ins = StringVar()

        self.conf_mot_de_passe_lb = Label(self,text = 'Confirmer mot de passe',bg = '#FFFFFF',font = ('arial', 10, 'bold'))

        self.conf_mot_de_passe_en = ttk.Entry(self, show = '*', textvariable=self.conf_mot_de_passe_ins)

        #partie bouton de connection-----------------------------------------------------------------------------------------------------

        self.valider_inscription_btn = ttk.Button(self, text = 'Valider',command  = self.valider)
        ...
        self.valider_inscription_btn.grid(row = 7, column = 1,columnspan = 2, pady = 5)
    ...
    def valider(self):
        ...
        if self.utilisateur_ins_en.get()!='' and self.mot_de_passe_ins_en.get()!='' and self.conf_mot_de_passe_en.get()!='':
            ...
            if self.mot_de_passe_ins_en.get() != self.conf_mot_de_passe_en.get():
                ...
                messagebox.showerror('Alert', 'Les mots de passes sont differents')
                ...
                messagebox.showerror('Alert', 'Mot de passe est faible \n veuiller depasser 8 caracteres')
                ...
                if test == True:
                    ...
                    mot_de_passe_securiser = hashlib.md5(self.mot_de_passe_ins_en.get().encode()).hexdigest()

                    bd.ajouter_objet('connexion', (self.utilisateur_ins_en.get(),mot_de_passe_securiser))
...
class fenetre3(Frame):
    ...
    def mainWidgets(self):
        ...
        self.inserer_lb = Label(self,text = 'Inserer Fichier',bg = '#FFFFFF',font = ('arial', 10, 'bold'))

        self.chemain = StringVar()

        self.inserer_en = ttk.Entry(self, textvariable=self.chemain)

        self.ouvrir_btn = ttk.Button(self, text = 'Ouvrir', command = self.fileINopen)
        ...
        self.choix1 = ttk.Radiobutton(self,text = 'Crypter',value = 'crypter',variable = self.choix, style = 'white.TRadiobutton', command = self.click_rbtn)

        self.choix2 = ttk.Radiobutton(self,text = 'Decrypter',value = 'decrypter',variable = self.choix, style = 'white.TRadiobutton', command = self.click_rbtn)
        ...
        self.cle_privee = ttk.Combobox(self, values = vals, textvariable = self.cle,state ='disable')

        #Traitement du self.choix-------------------------------------------------------------------------------------------------------------

        self.executer_btn = ttk.Button(self, text = 'Executer', command = self.executer,state = 'disable')

        #Affichange de l'historique

        self.historique_btn = ttk.Button(self, text = 'historique', command = self.historique)

        # Se deconnecter

        self.se_deconnecter_btn = ttk.Button(self, text = 'Se deconnecter', command = self.se_deconnecter)
    ...
    def historique(self):
        ...
        self.fenetre_historique = fenetre4(self, self.parent, bd, self.user, self.password)
        ...
        if self.chemain.get() != '':
            ...
            if self.choix.get() == 'crypter':
                ...
                try:
                    ...
                    chemin = filedialog.asksaveasfilename(confirmoverwrite=False, filetypes = data, defaultextension = data)
                    ...
                    private_key = bdcrypto(self.password, str(private_key)).encrypt()

                    bd.ajouter_objet('donnees_cryptage',(self.user, date, self.chemain.get(),str(private_key)))

                    messagebox.showinfo('info', 'Le cryptage est termine <^_^>' )
                    
                    self.clear_frame()

                    self.fenetre_cryptage = fenetre3(self.parent, bd, self.user, self.password)
                ...
                if self.cle_privee.get()  == '':
                    ...
                    if test == True:
                        ...
                        val = eval(bd.partie_objet_tableau('donnees_cryptage',self.user,3)[0][index-1])

                        private_key = eval(bdcrypto(self.password, '',val).decrypt())
                        ...
                        try:
                            ...
                            messagebox.showinfo('info', 'Le decryptage est termine <^_^>' )
                        ...
                        messagebox.showerror('Alert', 'Cle privee est imcompatible!')
...
class fenetre4(Toplevel):
    ...
    def mainWidgets(self):
        ...
        self.sbar = ttk.Scrollbar(self, orient="vertical", command = self.tableau.yview)

        self.tableau.grid(row = 1, column = 1,columnspan = 6,padx = (5,0), pady=(5, 0),sticky="news",rowspan = 5)
        ...
        self.supprimer_btn = ttk.Button(self, text = 'Supprimer', command = self.supprimer_element)
        ...
        for donnee in donnees:
            ...
            self.tableau.insert('', 'end', iid=donnee[1], values=(donnee[1], donnee[2]))
        ...
        if selected_items !=():
            ...
            if test == True:
                ...
                for element_selectionnee in selected_items:
                    ...
                    bd.supprimer_objet('donnees_cryptage', 'date', element_selectionnee)

                    self.tableau.delete(element_selectionnee)

                    self.manager.destroy()

                    self.fenetre3 = fenetre3(self.parent, bd, self.user, self.password)

        else:

            messagebox.showerror('Erreur', 'Vous devez selectionner un element')        
...
class bdcrypto():
    ...
    def encrypt(self):
        ...
        cipher_text, tag = cipher_config.encrypt_and_digest(bytes(self.texte, 'utf-8'))
